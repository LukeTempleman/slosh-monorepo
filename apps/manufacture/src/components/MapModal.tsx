...
const generateLocationData = (topMarkets: string[]) => {
  const saLocations = [
  ...
  return saLocations.map((location) => {
...
const MapModal: React.FC<MapModalProps> = ({ isOpen, onClose, brand }) => {
  const locationData = generateLocationData(brand.topMarkets);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-hidden">
        ...
        <div className="space-y-6">
          ...
          <GoogleMapsComponent
            title={`${brand.name} - South Africa Distribution`}
            defaultCenter={{ lat: -29.6100, lng: 28.2336 }}
            defaultZoom={6}
            markers={locationData.map(location => ({
              ...
              isTopMarket: location.isTopMarket
            }))}
          />
